...
/*   Updated: 2024/06/10 20:31:28 by gehovhan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <minirt.h>

bool	ft_validate_cone_util(t_token **tmp, t_list_token *list, char **error)
{
	*tmp = ft_jump(*tmp, 1);
...
bool	ft_validate_cone(t_list_token *list, char **error)
{
	...
	if (tmp && tmp->next && tmp->next->type == P_SEMI)
		return (set_error(error, ft_format_error(__func__, "")));
	if (!ft_validate_cone_util(&tmp, list, error))
		return (set_error(error, ft_format_error(__func__, "")));
	return (true);
}
